...
import org.crownj.params.RegTestParams;
...
import org.crownj.wallet.CoinSelection;
import org.crownj.wallet.CoinSelector;
import org.crownj.wallet.DeterministicKeyChain;
...
import org.crownj.core.CheckpointManager;
...
import org.crownj.core.FullPrunedBlockChain;
...
import org.crownj.core.listeners.BlocksDownloadedEventListener;
import org.crownj.core.listeners.DownloadProgressTracker;
import org.crownj.wallet.MarriedKeyChain;
import org.crownj.wallet.Protos;
import org.crownj.wallet.SendRequest;
import org.crownj.wallet.Wallet;
import org.crownj.wallet.WalletExtension;
import org.crownj.wallet.WalletProtobufSerializer;
...
import static org.crownj.core.Coin.parseCoin;
import static com.google.common.base.Preconditions.checkNotNull;

/**
 * A command line tool for manipulating wallets and working with crown.
 */
...
public class WalletTool implements Callable<Integer> {
    @CommandLine.Parameters(index = "0", description = "Action to perform. Valid values:%n" +
            ...
            "                       If --payment-request is specified, a transaction will be created using the provided payment request. A payment request can be a local file, a crown uri, or url to download the payment request, e.g.:%n" +
            "                         --payment-request=/path/to/my.crownpaymentrequest%n" +
            "                         --payment-request=crown:?r=http://merchant.com/pay.php?123%n" +
    ...
    @CommandLine.Option(names = "--addr", description ="Specifies a crown address, either SegWit or legacy.")
    ...
    @CommandLine.Option(names = "--fee-per-vkb", description = "Sets the network fee in crown per kilobyte when sending, e.g. --fee-per-vkb=0.0005")
    ...
    @CommandLine.Option(names = "--payment-request", description = "Specifies a payment request either by name of a local file, a crown uri, or url to download the payment request.")
    ...
    private static org.crown.protocols.payments.Protos.PaymentRequest paymentRequest;
    ...
    public static class Condition {
        ...
        public boolean matchcrowns(Coin comparison) {
            try {
    ...
    private void sendPaymentRequest(String location, boolean verifyPki) {
        if (location.startsWith("http") || location.startsWith("crown")) {
            try {
                ListenableFuture<PaymentSession> future;
                if (location.startsWith("http")) {
                    future = PaymentSession.createFromUrl(location, verifyPki);
                } else {
                    crownURI paymentRequestURI = new crownURI(location);
                    future = PaymentSession.createFromcrownUri(paymentRequestURI, verifyPki);
                }
                ...
                System.err.println("Invalid crown uri: " + e.getMessage());
                System.exit(1);
            ...
            try {
                paymentRequest = org.crown.protocols.payments.Protos.PaymentRequest.newBuilder().mergeFrom(stream).build();
            } catch(IOException e) {
    ...
    private void wait(WaitForEnum waitFor) throws BlockStoreException {
        ...
        switch (waitFor) {
            ...
            case BALANCE:
                ...
                if (condition.matchcrowns(wallet.getBalance(Wallet.BalanceType.ESTIMATED))) {
                    latch.countDown();
                    break;
                }
    ...
    private void deleteKey() {
        ...
        if (pubKeyStr != null) {
            ...
            try {
                ...
                System.err.println(addrStr + " does not parse as a crown address of the right network parameters.");
                return;
            }
        }
        if (key == null) {
    ...
    private synchronized void onChange(final CountDownLatch latch) {
        ...
        if (condition.matchcrowns(balance)) {
