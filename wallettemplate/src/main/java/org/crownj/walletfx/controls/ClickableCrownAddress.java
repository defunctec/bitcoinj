...
package org.crownj.walletfx.controls;
...
import org.crownj.walletfx.utils.QRCodeImages;
...
/**
 * A custom control that implements a clickable, copyable crown address. Clicking it opens a local wallet app. The
 * address looks like a blue hyperlink. Next to it there are two icons, one that copies to the clipboard and another
 * that shows a QRcode.
 */
public class ClickablecrownAddress extends AnchorPane implements OverlayController<ClickablecrownAddress> {
    ...
    @Override
    public void initOverlay(OverlayableStackPaneController overlayableStackPaneController, OverlayableStackPaneController.OverlayUI<? extends OverlayController<ClickablecrownAddress>> ui) {
        rootController = overlayableStackPaneController;
    }

    /**
     * @param theAppName The application name to use in crown URIs
     */
    ...
    public ClickablecrownAddress() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("crown_address.fxml"));
            loader.setRoot(this);
    ...
    public String uri() {
        return crownURI.convertTocrownURI(address.get(), null, appName, null);
    }
    ...
    protected void showQRCode(MouseEvent event) {
        ...
        final OverlayableStackPaneController.OverlayUI<ClickablecrownAddress> overlay = rootController.overlayUI(pane, this);
