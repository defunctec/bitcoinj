...
import org.crownj.core.PeerAddress;
...
import org.crownj.testing.*;
import org.crownj.utils.ExchangeRate;
import org.crownj.utils.Fiat;
import org.crownj.utils.Threading;
import org.crownj.wallet.Wallet.BalanceType;
...
import org.crownj.wallet.KeyChain.KeyPurpose;
import org.crownj.wallet.Protos.Wallet.EncryptionType;
...
import static org.crownj.core.Utils.HEX;
import static org.crownj.testing.FakeTxBuilder.*;
...
public class WalletTest extends TestWithWallet {
    ...
    private void basicSpendingCommon(Wallet wallet, Address toAddress, Address destination, Wallet encryptedWallet) throws Exception {
        ...
        // will attach a small fee. Because the crown protocol makes it difficult to determine the fee of an
    ...
    // identical result to crown Core. However the signatures are not deterministic - signing the same data
    ...
    public void blockChainCatchup() throws Exception {
        ...
        // What we'd really like to do is prove crown Core would accept it .... no such luck unfortunately.
    ...
    public void balances() throws Exception {
        ...
        Transaction send1 = wallet.createSend(OTHER_ADDRESS, valueOf(0, 10));
        // Reserialize.
        Transaction send2 = UNITTEST.getDefaultSerializer().makeTransaction(send1.crownSerialize());
    ...
    public void isConsistent_duplicates() throws Exception {
        ...
        Transaction txClone = UNITTEST.getDefaultSerializer().makeTransaction(tx.crownSerialize());
        try {
    ...
    public void doubleSpendUnspendsOtherInputs() throws Exception {
        ...
        send2 = UNITTEST.getDefaultSerializer().makeTransaction(send2.crownSerialize());
    ...
    public void doubleSpends() throws Exception {
        ...
        byte[] buf = send1.crownSerialize();
        buf[43] = 0;  // Break the signature: crownj won't check in SPV mode and this is easier than other mutations.
    ...
    public void pending1() throws Exception {
        ...
        final Transaction t1Copy = UNITTEST.getDefaultSerializer().makeTransaction(t1.crownSerialize());
    ...
    public void spendToSameWallet() throws Exception {
        ...
        assertEquals(ZERO, wallet.getBalance());
        sendMoneyToWallet(AbstractBlockChain.NewBlockType.BEST_CHAIN, outbound1);
        // We should have a balance of 1 CRW after the block is received.
        assertEquals(coin1, wallet.getBalance());
    }
    ...
    public void watchingScriptsBloomFilter() throws Exception {
        ...
        assertFalse(wallet.getBloomFilter(1e-12).contains(outPoint.unsafecrownSerialize()));

        sendMoneyToWallet(BlockChain.NewBlockType.BEST_CHAIN, t1);
        assertTrue(wallet.getBloomFilter(1e-12).contains(outPoint.unsafecrownSerialize()));
    }
    ...
    public void marriedKeychainBloomFilter() throws Exception {
        ...
        assertFalse(wallet.getBloomFilter(0.001).contains(outPoint.unsafecrownSerialize()));

        sendMoneyToWallet(BlockChain.NewBlockType.BEST_CHAIN, t1);
        assertTrue(wallet.getBloomFilter(0.001).contains(outPoint.unsafecrownSerialize()));
    }

    @Test
    public void autosaveImmediate() throws Exception {
        // Test that the wallet will save itself automatically when it changes.
        File f = File.createTempFile("crownj-unit-test", null);
    ...
    public void opReturnTwoOutputsTest() throws Exception {
        // Tests sending transaction where one output transfers CRW, the other one writes OP_RETURN.
        receiveATransaction(wallet, myAddress);
        Transaction tx = new Transaction(UNITTEST);
        Coin messagePrice = Coin.ZERO;
        Script script = ScriptBuilder.createOpReturnScript("hello world!".getBytes());
    ...
    public void feeSolverAndCoinSelectionTests2() throws Exception {
        ...
        assertTrue(request15.tx.unsafecrownSerialize().length > 1000);
        ...
        assertTrue(request16.tx.unsafecrownSerialize().length > 1000);
        ...
        int theoreticalMaxLength17 = request17.tx.unsafecrownSerialize().length + myKey.getPubKey().length + 75;
        ...
        Transaction spend17 = request17.tx;
        {
            ...
            final int length = spend17.unsafecrownSerialize().length;
        ...
        int theoreticalMaxLength18 = spend18.unsafecrownSerialize().length + myKey.getPubKey().length + 75;
        ...
        assertTrue(spend18.unsafecrownSerialize().length >= 998);
        assertTrue(spend18.unsafecrownSerialize().length <= 1001);
        ...
        request26.tx.addOutput(CENT.subtract(fee.add(dustThresholdMinusOne)),
                OTHER_ADDRESS);
        assertTrue(request26.tx.unsafecrownSerialize().length > 1000);
    ...
    public void totalReceivedSent() throws Exception {
        // Receive 4 CRW in 2 separate transactions
        ...
        // Send 3 CRW in a single transaction
