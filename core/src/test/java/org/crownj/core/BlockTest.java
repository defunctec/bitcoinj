...
import org.crownj.core.AbstractBlockChain.NewBlockType;
...
public class BlockTest {
    ...
    public void testWork() throws Exception {
        ...
        // This number is printed by crown Core at startup as the calculated value of chainWork on testnet:
    ...
    public void testHeaderParse() throws Exception {
        ...
        Block reparsed = TESTNET.getDefaultSerializer().makeBlock(header.crownSerialize());
        assertEquals(reparsed, header);
    }

    @Test
    public void testcrownSerialization() throws Exception {
        ...
        //
        // NB: This tests the crown serialization protocol.
        assertArrayEquals(block700000Bytes, block700000.crownSerialize());
    }
    ...
    public void testUpdateLength() {
        ...
        assertEquals(block.crownSerialize().length, block.length);
        final int origBlockLen = block.length;
        Transaction tx = new Transaction(UNITTEST);
        // this is broken until the transaction has > 1 input + output (which is required anyway...)
        //assertTrue(tx.length == tx.crownSerialize().length && tx.length == 8);
        ...
        assertEquals(tx.unsafecrownSerialize().length, tx.length);
        assertEquals(origTxLength, tx.length);
        block.addTransaction(tx);
        assertEquals(block.unsafecrownSerialize().length, block.length);
        ...
        assertEquals(block.length, block.unsafecrownSerialize().length);
        assertEquals(block.length, origBlockLen + tx.length);
        assertEquals(tx.length, origTxLength - 1);
    ...
    public void parseBlockWithHugeDeclaredTransactionsSize() throws Exception{
        Block block = new Block(UNITTEST, 1, Sha256Hash.ZERO_HASH, Sha256Hash.ZERO_HASH, 1, 1, 1, new ArrayList<Transaction>()) {
            ...
            public byte[] crownSerialize() {
                ...
                try {
                    crownSerializeToStream(baos);
                } catch (IOException e) {
                }
                return baos.toByteArray();
            }
        };
        byte[] serializedBlock = block.crownSerialize();
        try {
