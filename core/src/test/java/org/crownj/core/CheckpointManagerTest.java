...
public class CheckpointManagerTest {
    ...
    public void shouldThrowNullPointerExceptionWhenCheckpointsNotFound() throws IOException {
        expect(params.getId()).andReturn("org/crownj/core/checkpointmanagertest/notFound");
        replay(params);
    ...
    public void shouldThrowNullPointerExceptionWhenCheckpointsInUnknownFormat() throws IOException {
        expect(params.getId()).andReturn("org/crownj/core/checkpointmanagertest/unsupportedFormat");
        replay(params);
        new CheckpointManager(params, null);
    }
    ...
    public void shouldThrowIllegalStateExceptionWithNoCheckpoints() throws IOException {
        expect(params.getId()).andReturn("org/crownj/core/checkpointmanagertest/noCheckpoints");
        replay(params);
        new CheckpointManager(params, null);
    }
    ...
    public void canReadTextualStream() throws IOException {
        expect(params.getId()).andReturn("org/crownj/core/checkpointmanagertest/validTextualFormat");
        expect(params.getSerializer(false)).andReturn(
                new crownSerializer(params, NetworkParameters.ProtocolVersion.CURRENT.getcrownProtocolVersion(), false));
        expect(params.getProtocolVersionNum(NetworkParameters.ProtocolVersion.CURRENT))
                .andReturn(NetworkParameters.ProtocolVersion.CURRENT.getcrownProtocolVersion());
        replay(params);
        new CheckpointManager(params, null);
    }
}
