...
import org.crownj.core.Transaction.SigHash;
...
import static org.crownj.script.ScriptOpCodes.*;
 ...
 *    rethink your life. Seriously, why are you reimplementing crown consensus rules? Instead, go work on making
 *    crown Core consensus rules a shared library and use that. Seriously, you wont get it right, and starting with
 *    this tester as a way to try to do so will simply end in pain and lost coins. SERIOUSLY, JUST STOP!
 *
 * b) crown Core is failing some test in here and you're wondering what test is causing failure. Just stop. There is no
...
public class FullBlockTestGenerator {
    // Used by crowndComparisonTool and AbstractFullPrunedBlockChainTest to create test cases
    ...
    public RuleList getBlocksToTest(boolean runBarelyExpensiveTests, boolean runExpensiveTests, File blockStorageFile) throws ScriptException, ProtocolException, IOException {
            ...
            public boolean add(Rule element) {
                if (outStream != null && element instanceof BlockAndValidity) {
                    try {
                        ...
                        byte[] block = ((BlockAndValidity)element).block.crownSerialize();
        ...
        // Extend the b24 chain to make sure crownd isn't accepting b24
        ...
        // Extend the b26 chain to make sure crownd isn't accepting b26
        ...
        // Extend the b28 chain to make sure crownd isn't accepting b28
        ...
        Block b56;
        try {
            b56 = params.getDefaultSerializer().makeBlock(b57.block.crownSerialize());
        ...
        Block b56p2;
        try {
            b56p2 = params.getDefaultSerializer().makeBlock(b57p2.block.crownSerialize());
        } catch (ProtocolException e) {
            throw new RuntimeException(e); // Cannot happen.
        }
        ...
        Block b64; NewBlock b64Original;
        {
            ...
            for (Transaction transaction : b64Original.block.getTransactions())
                transaction.crownSerialize(stream);
            ...
            checkState(Arrays.equals(stream.toByteArray(), b64.crownSerialize()));
        ...
        Block b71 = params.getDefaultSerializer().makeBlock(b72.block.crownSerialize());
    ...
    class BlockAndValidity extends Rule {
        ...
        boolean sendOnce; // We can throw away the memory for this block once we send it the first time (if crownd asks again, its broken)
