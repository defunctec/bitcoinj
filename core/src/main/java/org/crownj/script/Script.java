 ...
 *
 * <p>crown transactions don't specify what they do directly. Instead <a href="https://en.crown.it/wiki/Script">a
...
public class Script {
    ...
    public byte[] getProgram() {
        try {
            // Don't round-trip as crown Core doesn't and it would introduce a mismatch.
            if (program != null)
     ...
     * crown Core does something similar.</p>
     */
    ...
    private int findSigInRedeem(byte[] signatureBytes, Sha256Hash hash) throws SignatureDecodeException {
        checkArgument(chunks.get(0).isOpCode()); // P2SH scriptSig
        int numKeys = Script.decodeFromOpN(chunks.get(chunks.size() - 2).opcode);
        TransactionSignature signature = TransactionSignature.decodeFromcrown(signatureBytes, true, false);
        for (int i = 0 ; i < numKeys ; i++) {
    ...
    private static boolean castToBool(byte[] data) {
        ...
        {
            // "Can be negative zero" - crown Core (see OpenSSL's BN_bn2mpi)
            if (data[i] != 0)
    ...
    // This is more or less a direct translation of the code in crown Core
    ...
    private static void executeCheckSig(Transaction txContainingThis, int index, Script script, LinkedList<byte[]> stack,
        ...
        try {
            TransactionSignature sig = TransactionSignature.decodeFromcrown(sigBytes, requireCanonical,
                verifyFlags.contains(VerifyFlag.LOW_S));
    ...
    private static int executeMultiSig(Transaction txContainingThis, int index, Script script, LinkedList<byte[]> stack,
        ...
        while (sigs.size() > 0) {
            ...
            try {
                TransactionSignature sig = TransactionSignature.decodeFromcrown(sigs.getFirst(), requireCanonical, false);
                Sha256Hash hash = txContainingThis.hashForSignature(index, connectedScript, (byte) sig.sighashFlags);
        ...
        // We uselessly remove a stack object to emulate a crown Core bug.
    ...
    public void correctlySpends(Transaction txContainingThis, int scriptSigIndex, @Nullable TransactionWitness witness, @Nullable Coin value,
        ...
        if (ScriptPattern.isP2WPKH(scriptPubKey)) {
            ...
            try {
                signature = TransactionSignature.decodeFromcrown(witness.getPush(0), true, true);
            ...
            TransactionSignature signature;
            try {
                signature = TransactionSignature.decodeFromcrown(chunks.get(0).data, true, true);
            } catch (SignatureDecodeException x) {
                throw new ScriptException(ScriptError.SCRIPT_ERR_SIG_DER, "Cannot decode", x);
            }
            ...
            TransactionSignature signature;
            try {
                signature = TransactionSignature.decodeFromcrown(chunks.get(0).data, false, false);
            } catch (SignatureDecodeException x) {
                throw new ScriptException(ScriptError.SCRIPT_ERR_SIG_DER, "Cannot decode", x);
            }
    ...
    public void correctlySpends(Transaction txContainingThis, long scriptSigIndex, Script scriptPubKey,
        ...
        try {
            txContainingThis = txContainingThis.getParams().getDefaultSerializer().makeTransaction(txContainingThis.crownSerialize());
