...
import org.crownj.utils.CRWAutoFormat.Style;
import static org.crownj.utils.CRWAutoFormat.Style.*;
 ...
 * representations of crown monetary values.</p>
 *
 ...
 * represents a crown monetary value.  Some of the features offered for doing this
 * are:</p>
 *
 * <ol>
 ...
 *       {@link CRWFormat#format(Object)} method. 
 *   <li>Parse a value by passing a {@code String}-type
 *       representation of it to the {@link CRWFormat#parse(String)} method.</ol>
 *
 * <p>For example, depending on your locale, values might be formatted
 * and parsed as follows:</p>
 *
 * <blockquote><pre>
 * CRWFormat f = CRWFormat.getInstance();
 * String c = f.format(Coin.COIN);                <strong>// "CRW 1.00"</strong>
 * String k = f.format(Coin.COIN.multiply(1000)); <strong>// "CRW 1,000.00"</strong>
 * String m = f.format(Coin.COIN.divide(1000));   <strong>// "mCRW 1.00"</strong>
 ...
 * be formatted, {@link CRWAutoFormat}, and another that formats any value in units of a
 * fixed, specified denomination, {@link CRWFixedFormat}.</p>
 *
 ...
 * values of {@link CRWAutoFormat.Style}.  There are two styles constants: {@link
 * CRWAutoFormat.Style#CODE} (the default), and {@link CRWAutoFormat.Style#SYMBOL}.  The
 * difference is that the {@code CODE} style uses an internationally-distinct currency
 * code, such as {@code "CRW"}, to indicate the units of denomination, while the
 * {@code SYMBOL} style uses a possibly-ambiguous currency symbol such as
 * {@code "฿"}.</p>
 *
 * <p>The denomination used when formatting will be either crown, millicoin
 ...
 * locale, a value of one crown might be formatted as {@code ฿1.00} where a value
 ...
 * increasing precision to convert the representation of a given quantity of crowns into a
 * representation of the same value denominated in the formatter's units.  For example, a scale
 * value of {@code 3} specifies a denomination of millicrowns, because to represent
 * {@code 1.0000 CRW}, or one crown, in millicrowns, one shifts the decimal point
 * three places, that is, to {@code 1000.0 mCRW}.</p>
 *
 * <h2>Construction</h2>
 *
 ...
 *   <li>Use a {@link CRWFormat.Builder} object.</ol>
 *
 ...
 * methods of this abstract base class {@link CRWFormat}.  There
 ...
 * automatic-denominating {@link CRWAutoFormat} instance for your
 ...
 * <blockquote><pre>
 * CRWFormat f = CRWFormat.getInstance();
 * String s = f.format(Coin.COIN); <strong>// "CRW 1.00"</strong>
 * </pre></blockquote>
 *
 ...
 * the {@link CRWFixedFormat} instance that is returned, and thus will
 * determine its denomination.  For example, if you want to format
 * values in units of microcrowns:</p>
 *
 * <blockquote><pre>CRWFormat m = CRWFormat.getInstance(6);
 ...
 *
 * <blockquote><pre>CRWFormat milliFormat = CRWFormat.getInstance(MILLICOIN_SCALE);</pre></blockquote>
 *
 * <p>Alternatively, if the type of the first argument to
 * {@code getInstance()} is one of the enumerated values of the
 * {@link CRWAutoFormat.Style} type, either {@link CRWAutoFormat.Style#CODE} or
 * {@link CRWAutoFormat.Style#SYMBOL}, then you will get a {@link CRWAutoFormat}
 * instance that uses either a currency code or symbol, respectively,
 * to indicate the results of its auto-denomination.</p>
 *
 * <blockquote><pre>
 * CRWFormat s = CRWFormat.getInstance(SYMBOL);
 ...
 * differently the same one-crown value:</p>
 *
 * <blockquote><pre>
 * <strong>// Next line returns "1,00 CRW"</strong>
 * CRWFormat.getInstance(Locale.GERMANY).format(Coin.COIN);
 * <strong>// Next line returns "1,00 ฿"</strong>
 * CRWFormat.getInstance(SYMBOL, Locale.GERMANY).format(Coin.COIN);
 * <strong>// Next line returns "1.000,00"</strong>
 * CRWFormat.getMilliInstance(Locale.GERMANY).format(Coin.COIN);
 * <strong>// Next line returns "10.000,00"</strong>
 * CRWFormat.getInstance(4, Locale.GERMANY).format(Coin.COIN);
 * </pre></blockquote>
 *
 ...
 *
 * <p>A new {@link CRWFormat.Builder} instance is returned by the {@link #builder()} method.  Such
 * an object has methods that set the configuration parameters of a {@link CRWFormat}
 * object.  Its {@link Builder#build()} method constructs and returns a {@link CRWFormat} instance
 ...
 *
 * <blockquote><pre>
 * CRWFormat f = CRWFormat.builder().style(CODE).code("XBT").build();
 * String out = f.format(COIN); <strong>// "XBT 1.00"</strong>
 * </pre></blockquote>
 *
 * <p>See the documentation of the {@link CRWFormat.Builder} class for details.</p>
 *
 * <h2>Formatting</h2>
 *
 * <p>You format a crown monetary value by passing it to the {@link CRWFormat#format(Object)}
 ...
 * number of crowns.  A value having a fractional amount of satoshis is rounded to the
 ...
 * {@link CRWFixedFormat} also accept a variable-length sequence of additional {@code int}
 ...
 *
 * <blockquote><pre>
 * CRWFormat f = CRWFormat.getCoinInstance();
 ...
 * {@link CRWFixedFormat#REPEATING_PLACES}, {@link
 * CRWFixedFormat#REPEATING_DOUBLETS} and {@link
 * CRWFixedFormat#REPEATING_TRIPLETS}.  These signify repeating groups
 ...
 *
 * <blockquote><pre>
 * CRWFormat de = CRWFormat.getInstance(Locale.GERMANY);
 * FieldPosition currField = new FieldPosition(NumberFormat.Field.CURRENCY);
 * <strong>// next line formats the value as "987.654.321,23 µCRW"</strong>
 * String output = de.format(valueOf(98765432123L), new StringBuffer(), currField);
 * <strong>// next line sets variable currencyCode to "µCRW"</strong>
 ...
 *
 * <blockquote><pre>
 * CRWFixedFormat kilo = (CRWFixedFormat)CRWFormat(-3); <strong>// scale -3 for kilocoins</strong>
 * Coin value = Coin.parseCoin("1230");
 * <strong>// variable coded will be set to "kCRW 1.23"</strong>
 ...
 * CRWFormat(4).code(); <strong>// unnamed denomination has no code; raises exception</strong>
 * </pre></blockquote>
 *
 ...
 *                SATOSHI};
 * CRWFormat f = CRWFormat.getCoinInstance(2, REPEATING_PLACES);
 ...
 * CRWFixedFormat} and {@link CRWAutoFormat} is analogous to the difference in formatting
 * behavior between instances of those classes.  Instances of {@link CRWAutoFormat} recognize
 ...
 * instances of {@link CRWFixedFormat} by default recognize no codes nor symbols, but rather
 ...
 * CRWFixedFormat} can be overridden by setting a parsing pattern that includes a currency sign
 * using the {@link CRWFormat.Builder#pattern()} method.</p>
 *
 * <p>The {@link CRWAutoFormat#parse(String)} method of {@link CRWAutoFormat} (and of
 * {@link CRWAutoFormat} configured with applicable non-default pattern) will recognize a
 ...
 * {@code µCRW} or other appropriate permutations of those characters.  Additionally, if
 * either or both of a custom currency code or symbol is configured using {@link
 * CRWFormat.Builder#code} or {@link CRWFormat.Builder#code}, then such code or symbol will
 ...
 * {@code CRWFormat} instance, on the other hand, whether or not the units indicator must
 * be separated by a space from the number is determined by the locale.  The {@link
 * CRWFormat#pattern()} method returns a representation of the pattern that
 * can be examined to determine whether a space must separate currency signs from numbers in
 * parsed {@link String}s.</p>
 *
 * <p>When parsing, if the currency-units indicator is absent, then a {@link CRWAutoFormat}
 * instance will infer a denomination of crowns while a {@link CRWFixedFormat} will infer the
...
public abstract class CRWFormat extends Format {
    ...
    /** The conventional international currency code for crowns: "CRW" */
    private static final String COIN_CODE = "CRW";
    /** The default currency symbols for crowns */
    ...
    /* Scale is the number of decimal-places difference from same value in crowns */
    /** A constant useful for specifying a denomination of crowns, the {@code int} value
     *  {@code 0}. */
    public static final int COIN_SCALE = 0;

    /** A constant useful for specifying a denomination of millicrowns, the {@code int}
     *  value {@code 3}. */
    public static final int MILLICOIN_SCALE = 3;

    /** A constant useful for specifying a denomination of microcrowns, the {@code int}
     *  value {@code 6}. */
    ...
    /**
     * <p>This class constructs new instances of {@link CRWFormat}, allowing for the
     * configuration of those instances before they are constructed.  After obtaining a
     * {@code Builder} object from the {@link CRWFormat#builder()} method, invoke the
     * necessary setter methods to obtain your desired configuration.  Finally, the {@link
     * #build()} method returns a new {@code CRWFormat} object that has the specified
     * configuration.
     *
     * <p>All the setter methods override defaults.  Invoking {@code build()} without invoking any
     * of the setting methods is equivalent to invoking {@link CRWFormat#getInstance()} with no arguments.
     *
    ...
    public static class Builder {
        ...
        private enum Variant {
            AUTO { @Override CRWFormat newInstance(Builder b) {
                       return getInstance(b.style, b.locale, b.minimumFractionDigits);
                   }},
            FIXED,
            UNSET;
            CRWFormat newInstance(Builder b) {
        ...
        private Style style = CRWAutoFormat.Style.CODE;
        private int scale = 0;
        private String symbol = "",code = "",pattern = "",localizedPattern = "";

        private Builder() {}

        /** Specify the new {@code CRWFormat} is to be automatically-denominating.
         * The argument determines which of either codes or symbols the new {@code CRWFormat}
        ...
        public Builder style(CRWAutoFormat.Style val) {
         ...
         * appropriately for the context of generating {@link CRWAutoFormat} instances.
         *
         ...
         *  denominated in crowns.  For example, a denomination of millicrowns is specified
         *  with a value of {@code 3}.
         *
         * <p>The {@code CRWFormat} class provides appropriately named
         * {@code int}-type constants for the three common values, {@link CRWFormat#COIN_SCALE},
         * {@link CRWFormat#MILLICOIN_SCALE} {@link CRWFormat#MICROCOIN_SCALE}.
         *
         * <p>If neither this method nor {@link #style(CRWAutoFormat.Style)} is invoked on a
         * {@link Builder}, then the {@link CRWFormat} will default to a
         * fixed-denomination of crowns, equivalent to invoking this method with an argument
         * of {@code 0}. */
         ...
         *  class.
         *
         *  <p>The pattern is localized according to the locale of the {@code CRWFormat}
         *  instance, the symbols for which can be examined by inspecting the {@link
         *  java.text.DecimalFormatSymbols} object returned by {@link CRWFormat#symbols()}.
        ...
        /** Return a new {@link CRWFormat} instance.  The object returned will be configured according
         *  to the state of this {@code Builder} instance at the time this method is invoked. */
        public CRWFormat build() {
            CRWFormat f = variant.newInstance(this);
    ...
    protected CRWFormat(DecimalFormat numberFormat, int minDecimals, List<Integer> groups) {
     ...
     * currency code, for example "{@code CRW}", to indicate that denomination.  The
     ...
     */
    public static CRWFormat getInstance() { return getInstance(defaultLocale()); }
     ...
     */
    public static CRWFormat getSymbolInstance() { return getSymbolInstance(defaultLocale()); }

    /**
     * Return a new auto-denominating instance that will indicate units using a currency
     * code, for example, {@code "CRW"}.  Formatting and parsing will be done
     * according to the default locale.
     */
    public static CRWFormat getCodeInstance() { return getCodeInstance(defaultLocale()); }
     ...
     * locale.
     */
    public static CRWFormat getSymbolInstance(int fractionPlaces) {
     ...
     * {@code "CRW"}.  The returned object will format the fraction-part of numbers using
     * the given number of decimal places, or fewer as necessary to avoid giving a place to
     * fractional satoshis.  Formatting and parsing will be done according to the default
     * locale.
     */
    public static CRWFormat getCodeInstance(int minDecimals) {
     ...
     * units using a currency code, for example, {@code "mCRW"}.
     */
    public static CRWFormat getInstance(Locale locale) { return getCodeInstance(locale); }
     ...
     * units using a currency code, for example, {@code "mCRW"}.
     */
    public static CRWFormat getCodeInstance(Locale locale) { return getInstance(CODE, locale); }
     ...
     * {@code "mCRW"}.  The returned object will format the fraction-part of numbers using
     * the given number of decimal places, or fewer as necessary to avoid giving a place to
     * fractional satoshis.
     */
    public static CRWFormat getInstance(Locale locale, int minDecimals) {
     ...
     * fractional satoshis.
     */
    public static CRWFormat getCodeInstance(Locale locale, int minDecimals) {
     ...
     */
    public static CRWFormat getSymbolInstance(Locale locale) {
     ...
     * fractional satoshis.
     */
    public static CRWFormat getSymbolInstance(Locale locale, int fractionPlaces) {
     ...
     * {@code "฿"}, or code, such as {@code "mCRW"}, depending on the value of
     * the argument.  Formatting and parsing will be done according to the default locale.
     */
    public static CRWFormat getInstance(Style style) { return getInstance(style, defaultLocale()); }
     ...
     * {@code "mCRW"}, depending on the value of the first argument.  The returned object
     ...
     */
    public static CRWFormat getInstance(Style style, int fractionPlaces) {
     ...
     * {@code "CRW"}, or symbol, such as {@code "฿"}, depending on the value
     ...
     */
    public static CRWFormat getInstance(Style style, Locale locale) {
     ...
     * such as {@code "mCRW"}, or symbol, such as {@code "฿"},
     ...
     * fractional satoshis.
     */
    public static CRWFormat getInstance(Style style, Locale locale, int fractionPlaces) {
        return new CRWAutoFormat(locale, style, fractionPlaces);
    }
     ...
     */
    public static CRWFormat getCoinInstance() { return getCoinInstance(defaultLocale()); }
     ...
     */
    public static CRWFormat getCoinInstance(int minFractionPlaces, int... groups) {
     ...
     */
    public static CRWFormat getCoinInstance(Locale locale) {
        return getInstance(COIN_SCALE, locale, 2);
    }

    /**
     * Return a newly-constructed instance for the given locale that will format
     * values in terms of crowns, with the given minimum number of fractional
     ...
     */
    public static CRWFormat getCoinInstance(Locale locale, int scale, int... groups) {
     ...
     */
    public static CRWFormat getMilliInstance() { return getMilliInstance(defaultLocale()); }
     ...
     * necessary.
     */
    public static CRWFormat getMilliInstance(Locale locale) {
     ...
     */
    public static CRWFormat getMilliInstance(int scale, int... groups) {
     ...
     */
    public static CRWFormat getMilliInstance(Locale locale, int scale, int... groups) {
     ...
     * necessary.
     */
    public static CRWFormat getMicroInstance() { return getMicroInstance(defaultLocale()); }
     ...
     * necessary.
     */
    public static CRWFormat getMicroInstance(Locale locale) {
     ...
     * necessary to avoid rounding, to a limiting precision of satoshis.
     */
    public static CRWFormat getMicroInstance(int scale, int... groups) {
     ...
     * limiting precision of satoshis.
     */
    public static CRWFormat getMicroInstance(Locale locale, int scale, int... groups) {
     ...
     * limiting precision of satoshis.
     */
    public static CRWFormat getInstance(int scale, int minDecimals, int... groups) {
     ...
     */
    public static CRWFormat getInstance(int scale) {
     ...
     */
    public static CRWFormat getInstance(int scale, Locale locale) {
     ...
     */
    public static CRWFormat getInstance(int scale, Locale locale, int minDecimals, int... groups) {
     ...
     */
    public static CRWFormat getInstance(int scale, Locale locale, int minDecimals, List<Integer> groups) {
        return new CRWFixedFormat(locale, scale, minDecimals, groups);
    }

    // ****** FORMATTING *****

    /**
     * Formats a crown monetary value and returns an {@link AttributedCharacterIterator}.
    ...
    /**
     * Formats a crown value as a number and possibly a units indicator and appends the
     ...
     * number of crowns, rounded to the nearest satoshi as necessary.
     *
    ...
    /**
     * Formats a crown value as a number and possibly a units indicator to a
     ...
     * {@link BigDecimal} is interpreted as that number of crowns, rounded to the
     ...
     * is the size of the decimal-place shift between the given crown-value denominated in
     * crowns and that same value as formatted.  A fixed-denomination formatter will ignore
     * the arguments.
     *
    ...
    /**
     * Takes a crown monetary value that the client wants to format and returns the number of
    ...
    /**
     * Takes an object representing a crown quantity of any type the
    ...
    private static BigInteger inSatoshis(Object qty) {
        BigInteger satoshis;
        // the value might be crowns or satoshis
    ...
    /**
      * Parse a {@link String} representation of a crown monetary value.  Returns a
    ...
    /** Parse a {@link String} representation of a crown monetary value.  If this
     * object's pattern includes a currency sign, either symbol or code, as by default is true
     * for instances of {@link CRWAutoFormat} and false for instances of {@link
     * CRWFixedFormat}, then denominated (i.e., prefixed) currency signs in the parsed String
     ...
     * String, then the number is interpreted as a quatity of crowns.
     * <p>If the pattern contains neither a currency symbol nor sign, then instances of {@link
     * CRWAutoFormat} will interpret the parsed number as a quantity of crowns, and instances
     * of {@link CRWAutoFormat} will interpret the number as a quantity of that instance's
     * configured denomination, which can be ascertained by invoking the {@link
     * CRWFixedFormat#symbol()} or {@link CRWFixedFormat#code()} method.
     *
    ...
    /** Return the unprefixed currency symbol for crowns configured for this object.  The
    ...
    /** Return the unprefixed international currency code for crowns configured for this
    ...
    @Override public boolean equals(Object o) {
        ...
        if (!(o instanceof CRWFormat)) return false;
        CRWFormat other = (CRWFormat)o;
