...
import org.crownj.core.AddressFormatException;
import org.crownj.core.Coin;
import org.crownj.core.NetworkParameters;
import org.crownj.params.AbstractcrownNetParams;
...
/**
 * <p>Provides a standard implementation of a crown URI with support for the following:</p>
 *
 * <ul>
 ...
 *
 * <ul>
 * <li>{@code crown:<address>}</li>
 * <li>{@code crown:<address>?<name1>=<value1>&<name2>=<value2>} with multiple
 ...
 * @see <a href="https://github.com/crown/bips/blob/master/bip-0021.mediawiki">BIP 0021</a>
 */
public class crownURI {
    ...
    /**
     * URI for crown network. Use {@link AbstractcrownNetParams#crown_SCHEME} if you specifically
     * need crown, or use {@link NetworkParameters#getUriScheme} to get the scheme
     * from network parameters.
     */
    @Deprecated
    public static final String crown_SCHEME = "crown";
    ...
    /**
     * Constructs a new crownURI from the given string. Can be for any network.
     *
     * @param uri The raw URI data to be parsed (see class comments for accepted formats)
     * @throws crownURIParseException if the URI is not syntactically or semantically valid.
     */
    public crownURI(String uri) throws crownURIParseException {
        this(null, uri);
    }

    /**
     * Constructs a new object by trying to parse the input as a valid crown URI.
     *
     ...
     *
     * @throws crownURIParseException If the input fails crown URI syntax and semantic checks.
     */
    public crownURI(@Nullable NetworkParameters params, String input) throws crownURIParseException {
        checkNotNull(input);

        String scheme = null == params
            ? AbstractcrownNetParams.crown_SCHEME
            : params.getUriScheme();
        ...
        try {
            ...
            throw new crownURIParseException("Bad URI syntax", e);
        }

        // URI is formed as  crown:<address>?<query parameters>
        // blockchain.info generates URIs of non-BIP compliant form crown://address?....
        // We support both until Ben fixes his code.
        
        // Remove the crown scheme.
        // (Note: getSchemeSpecificPart() is not used as it unescapes the label and parse then fails.
        // For instance with : crown:129mVqKUmJ9uwPxKJBnNdABbuaaNfho4Ha?amount=0.06&label=Tom%20%26%20Jerry
        ...
        if (inputLc.startsWith(blockchainInfoScheme)) {
            ...
            throw new crownURIParseException("Unsupported URI scheme: " + uri.getScheme());
        }

        // Split off the address from the rest of the query parameters.
        String[] addressSplitTokens = schemeSpecificPart.split("\\?", 2);
        if (addressSplitTokens.length == 0)
            throw new crownURIParseException("No data found after the crown: prefix");
        ...
        if (!addressToken.isEmpty()) {
            // Attempt to parse the addressToken as a crown address for this network
            try {
                Address address = Address.fromString(params, addressToken);
                putWithValidation(FIELD_ADDRESS, address);
            } catch (final AddressFormatException e) {
                throw new crownURIParseException("Bad address", e);
            }
        }

        if (addressToken.isEmpty() && getPaymentRequestUrl() == null) {
            throw new crownURIParseException("No address and no r= parameter found");
        }
    }
     ...
     */
    private void parseParameters(@Nullable NetworkParameters params, String addressToken, String[] nameValuePairTokens) throws crownURIParseException {
        // Attempt to decode the rest of the tokens into a parameter map.
        for (String nameValuePairToken : nameValuePairTokens) {
            final int sepIndex = nameValuePairToken.indexOf('=');
            if (sepIndex == -1)
                throw new crownURIParseException("Malformed crown URI - no separator in '" +
                        nameValuePairToken + "'");
            if (sepIndex == 0)
                throw new crownURIParseException("Malformed crown URI - empty name '" +
                        nameValuePairToken + "'");
            ...
            if (FIELD_AMOUNT.equals(nameToken)) {
                ...
                try {
                    ...
                    if (params != null && amount.isGreaterThan(params.getMaxMoney()))
                        throw new crownURIParseException("Max number of coins exceeded");
                    if (amount.signum() < 0)
     ...
     */
    private void putWithValidation(String key, Object value) throws crownURIParseException {
        if (parameterMap.containsKey(key)) {
            throw new crownURIParseException(String.format(Locale.US, "'%s' is duplicated, URI is invalid", key));
        } else {
    ...
    /**
     * The crown address from the URI, if one was present. It's possible to have crown URI's with no address if a
     ...
     *         10,000,000 units is 1 CRW. May be null if no amount is specified
     */
    ...
    public String toString() {
        StringBuilder builder = new StringBuilder("crownURI[");
        boolean first = true;
    ...
    /**
     * Simple crown URI builder using known good fields.
     *
     * @param address The crown address
     * @param amount The amount
     * @param label A label
     * @param message A message
     * @return A String containing the crown URI
     */
    public static String convertTocrownURI(Address address, Coin amount,
                                             String label, String message) {
        return convertTocrownURI(address.getParameters(), address.toString(), amount, label, message);
    }
     ...
     * @return A String containing the crown URI
     */
    public static String convertTocrownURI(NetworkParameters params,
