...
import org.crownj.utils.MonetaryFormat;
...
/**
 * Represents a monetary crown value. This class is immutable.
 */
public final class Coin implements Monetary, Comparable<Coin>, Serializable {

    /**
     * Number of decimals for one crown. This constant is useful for quick adapting to other coins because a lot of
     * constants derive from it.
     */
    public static final int SMALLEST_UNIT_EXPONENT = 8;

    /**
     * The number of satoshis equal to one crown.
     */
    ...
    /**
     * 0.01 crowns. This unit is not really used much.
     */
    public static final Coin CENT = COIN.divide(100);

    /**
     * 0.001 crowns, also known as 1 mCRW.
     */
    public static final Coin MILLICOIN = COIN.divide(1000);

    /**
     * 0.000001 crowns, also known as 1 µCRW or 1 uCRW.
     */
    public static final Coin MICROCOIN = MILLICOIN.divide(1000);

    /**
     * A satoshi is the smallest unit that can be transferred. 100 million of them fit into a crown.
     */
     ...
     *
     * @param coins Number of crowns
     * @param cents Number of bitcents (0.01 crown)
     * @return {@code Coin} object containing value in satoshis
     */
    ...
    /**
     * Convert a decimal amount of CRW into satoshis.
     *
     * @param coins number of coins
     * @return number of satoshis
     */
    public static long CRWToSatoshi(BigDecimal coins) {
        return coins.movePointRight(SMALLEST_UNIT_EXPONENT).longValueExact();
    }

    /**
     * Convert an amount in satoshis to an amount in CRW.
     *
     * @param satoshis number of satoshis
     * @return number of crowns (in CRW)
     */
    public static BigDecimal satoshiToCRW(long satoshis) {
        return new BigDecimal(satoshis).movePointLeft(SMALLEST_UNIT_EXPONENT);
    }

    /**
     * Create a {@code Coin} from a decimal amount of CRW.
     *
     * @param coins number of coins (in CRW)
     * @return {@code Coin} object containing value in satoshis
     */
    public static Coin ofCRW(BigDecimal coins) {
        return Coin.valueOf(CRWToSatoshi(coins));
    }
    ...
    public static Coin parseCoin(final String str) {
        try {
            long satoshis = CRWToSatoshi(new BigDecimal(str));
            return Coin.valueOf(satoshis);
    ...
    /**
     * Convert to number of crown (in CRW)
     *
     * @return decimal number of crown (in CRW)
     */
    public BigDecimal toCRW() {
        return satoshiToCRW(this.value);
    }

    private static final MonetaryFormat FRIENDLY_FORMAT = MonetaryFormat.CRW.minDecimals(2).repeatOptionalDecimals(1, 6).postfixCode();
    ...
    private static final MonetaryFormat PLAIN_FORMAT = MonetaryFormat.CRW.minDecimals(0).repeatOptionalDecimals(1, 8).noCode();

    /**
     * <p>
     * Returns the value as a plain string denominated in CRW.
     * The result is unformatted with no trailing zeroes.
     * For instance, a value of 150000 satoshis gives an output string of "0.0015" CRW
     * </p>
     */
