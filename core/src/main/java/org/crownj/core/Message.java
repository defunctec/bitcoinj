...
/**
 * <p>A Message is a data structure that can be serialized/deserialized using the crown serialization format.
...
public abstract class Message {
    ...
    // These methods handle the serialization/deserialization using the custom crown protocol.
    ...
    /**
     * Returns a copy of the array returned by {@link Message#unsafecrownSerialize()}, which is safe to mutate.
    ...
    public byte[] crownSerialize() {
        byte[] bytes = unsafecrownSerialize();
    ...
    /**
     * <p>Serialize this message to a byte array that conforms to the crown wire protocol.</p>
     *
     ...
     */
    public byte[] unsafecrownSerialize() {
        ...
        if (serializer.isParseRetainMode()) {
            ...
            // which means another call to crownSerialize is coming.  If we didn't recache then internal
    ...
    /**
     * Serialize this message to the provided OutputStream using the crown wire format.
     *
     * @param stream
     * @throws IOException
     */
    public final void crownSerialize(OutputStream stream) throws IOException {
    ...
    /**
     * Serializes this message to the provided stream. If you just want the raw bytes use crownSerialize().
     */
    protected void crownSerializeToStream(OutputStream stream) throws IOException {
        log.error("Error: {} class has not implemented crownSerializeToStream method.  Generating message with no payload", getClass());
    }

    /**
     * This method is a NOP for all classes except Block and Transaction.  It is only declared in Message
     * so crownSerializer can avoid 2 instanceof checks + a casting.
     */
