/*
 * Copyright 2014 the crownj authors
 *
...
/**
 * <p>This command is supported only by <a href="http://github.com/crownxt/crownxt">crown XT</a> nodes, which
 ...
 * <a href="https://github.com/crown/bips/blob/master/bip-0065.mediawiki">BIP 65</a> for more information on this
 * message.</p>
 *
 * <p>Note that this message does not let you query the UTXO set by address, script or any other criteria. The
 * reason is that crown nodes don't calculate the necessary database indexes to answer such queries, to save
...
public class GetUTXOsMessage extends Message {
    ...
    protected void crownSerializeToStream(OutputStream stream) throws IOException {
        ...
        for (TransactionOutPoint outPoint : outPoints) {
            outPoint.crownSerializeToStream(stream);
        }
    }
