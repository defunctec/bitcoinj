...
import org.crownj.core.TransactionConfidence.ConfidenceType;
import org.crownj.crypto.TransactionSignature;
import org.crownj.script.Script;
import org.crownj.script.Script.ScriptType;
import org.crownj.script.ScriptBuilder;
import org.crownj.script.ScriptError;
import org.crownj.script.ScriptException;
import org.crownj.script.ScriptOpCodes;
import org.crownj.script.ScriptPattern;
import org.crownj.signers.TransactionSigner;
import org.crownj.utils.ExchangeRate;
import org.crownj.wallet.Wallet;
import org.crownj.wallet.WalletTransaction.Pool;
...
import static org.crownj.core.NetworkParameters.ProtocolVersion.WITNESS_VERSION;
import static org.crownj.core.Utils.*;
 ...
 * the minting of new coins. A Transaction object corresponds to the equivalent in the crown C++ implementation.</p>
 *
 * <p>Transactions are the fundamental atoms of crown and have many powerful features. Read
 * <a href="https://crownj.github.io/working-with-transactions">"Working with transactions"</a> in the
 * documentation to learn more about how to use this class.</p>
 *
 * <p>All crown transactions are at risk of being reversed, though the risk is much less than with traditional payment
...
public class Transaction extends ChildMessage {
    ...
    /**
     * When this bit is set in protocolVersion, do not include witness. The actual value is the same as in crown Core
     * for consistency.
     */
    ...
    /**
     * If feePerKb is lower than this, crown Core will treat it as if there were no fee.
     */
    public static final Coin REFERENCE_DEFAULT_MIN_TX_FEE = Coin.valueOf(1000); // 0.01 mCRW

    /**
     * If using this feePerKb, transactions will get confirmed within the next couple of blocks.
     * This should be adjusted from time to time. Last adjustment: February 2017.
     */
    public static final Coin DEFAULT_TX_FEE = Coin.valueOf(100000); // 1 mCRW
     ...
     * @param payload crown protocol formatted byte array containing message content.
     ...
     * @param hashFromHeader Used by crownSerializer. The serializer has to calculate a hash for checksumming so to
    ...
    public Sha256Hash getTxId() {
        if (cachedTxId == null) {
            if (!hasWitnesses() && cachedWTxId != null) {
                ...
                try {
                    crownSerializeToStream(stream, false);
                } catch (IOException e) {
    ...
    private boolean allowWitness() {
        ...
        return (protocolVersion & SERIALIZE_TRANSACTION_NO_WITNESS) == 0
                && protocolVersion >= WITNESS_VERSION.getcrownProtocolVersion();
    }
    ...
    public Sha256Hash getWTxId() {
        if (cachedWTxId == null) {
            if (!hasWitnesses() && cachedTxId != null) {
                ...
                try {
                    crownSerializeToStream(baos, hasWitnesses());
                } catch (IOException e) {
                    throw new RuntimeException(e); // cannot happen
                }
    ...
    public int getWeight() {
        ...
        try (final ByteArrayOutputStream stream = new UnsafeByteArrayOutputStream(length)) {
            ...
            stream.reset();
            crownSerializeToStream(stream, true);
    ...
    /**
     * Deserialize according to <a href="https://github.com/crown/bips/blob/master/bip-0144.mediawiki">BIP144</a> or
     * the <a href="https://en.crown.it/wiki/Protocol_documentation#tx">classic format</a>, depending on if the
     ...
     * value is determined by a formula that all implementations of crown share. In 2011 the value of a coinbase
    ...
    public String toString(@Nullable AbstractBlockChain chain, @Nullable CharSequence indent) {
        ...
        int size = unsafecrownSerialize().length;
        int vsize = getVsize();
    ...
    /**
     * Serializes the transaction into the crown network format and encodes it as hex string.
     * @return raw transaction in hex format
     */
    public String toHexString() {
        return Utils.HEX.encode(unsafecrownSerialize());
    }
    ...
    public void clearInputs() {
        ...
        this.length = this.unsafecrownSerialize().length;
    }
     ...
     * and requires external synchronization. Please refer to general documentation on crown scripting and contracts
    ...
    public Sha256Hash hashForSignature(int inputIndex, byte[] connectedScript, byte sigHashType) {
        ...
        //
        //   https://en.crown.it/wiki/Contracts

        try {
            // Create a copy of this transaction to operate upon because we need make changes to the inputs and outputs.
            // It would not be thread-safe to change the attributes of the transaction object itself.
            Transaction tx = this.params.getDefaultSerializer().makeTransaction(this.crownSerialize());
            ...
            // This step has no purpose beyond being synchronized with crown Core's bugs. OP_CODESEPARATOR
            // is a legacy holdover from a previous, broken design of executing scripts that shipped in crown 0.1.
            ...
            // Set the input to the script of its output. crown Core does this but the step has no obvious purpose as
            ...
            if ((sigHashType & 0x1f) == SigHash.NONE.value) {
                ...
                if (inputIndex >= tx.outputs.size()) {
                    ...
                    // crown implementation. crown Core also contains a bug in handling this case:
                    // any transaction output that is signed in this case will result in both the signed output
                    // and any future outputs to this public key being steal-able by anyone who has
                    // the resulting signature and the public key (both of which are part of the signed tx input).

                    // crown Core's bug is that SignatureHash was supposed to return a hash and on this codepath it
            ...
            tx.crownSerializeToStream(bos, false);
     ...
     * scriptPubKey of the output you're signing for. (See BIP143: https://github.com/crown/bips/blob/master/bip-0143.mediawiki)</p>
     *
     * @param inputIndex   input the signature is being calculated for. Tx signatures are always relative to an input.
    ...
    @Override
    protected void crownSerializeToStream(OutputStream stream) throws IOException {
        boolean useSegwit = hasWitnesses() && allowWitness();
        crownSerializeToStream(stream, useSegwit);
    }

    /**
     * Serialize according to <a href="https://github.com/crown/bips/blob/master/bip-0144.mediawiki">BIP144</a> or the
     * <a href="https://en.crown.it/wiki/Protocol_documentation#tx">classic format</a>, depending on if segwit is
     * desired.
     */
    protected void crownSerializeToStream(OutputStream stream, boolean useSegwit) throws IOException {
        // version
        ...
        for (TransactionInput in : inputs)
            in.crownSerialize(stream);
        // txout_count, txouts
        stream.write(new VarInt(outputs.size()).encode());
        for (TransactionOutput out : outputs)
            out.crownSerialize(stream);
        // script_witnisses
        if (useSegwit) {
            for (TransactionInput in : inputs) {
                in.getWitness().crownSerializeToStream(stream);
            }
        }
        // lock_time
     ...
     * since crown 0.8+ a transaction that did not end its lock period (non final) is considered to be non
     ...
     * (<a href="https://github.com/crown/bips/blob/master/bip-0068.mediawiki">BIP 68</a>) if it is version 2 or
     ...
     * <a href="https://github.com/crown/bips/blob/master/bip-0125.mediawiki">full replace-by-fee </a> semantics.
     */
     ...
     * This is useful in certain types of <a href="http://en.crown.it/wiki/Contracts">contracts</a>, such as
     * micropayment channels.</p>
     *
     * <p>Note that currently the replacement feature is disabled in crown Core and will need to be
