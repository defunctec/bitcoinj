...
import org.crownj.net.AbstractTimeoutHandler;
import org.crownj.net.NioClient;
import org.crownj.net.NioClientManager;
import org.crownj.net.StreamConnection;
import org.crownj.store.BlockStore;
import org.crownj.store.BlockStoreException;
import org.crownj.utils.ListenerRegistration;
...
/**
 * <p>A Peer handles the high level communication with a crown node, extending a {@link PeerSocketHandler} which
...
public class Peer extends PeerSocketHandler {
    ...
    private void processVersionMessage(VersionMessage peerVersionMessage) throws ProtocolException {
        ...
        log.info(toString());
        // crownj is a client mode implementation. That means there's not much point in us talking to other client
        ...
        if ((peerVersionMessage.localServices & VersionMessage.NODE_crown_CASH) == VersionMessage.NODE_crown_CASH) {
        ...
        try {
            ...
            try {
                if (awaitingFreshFilter != null) {
                    ...
                    // range of blocks and get an inv both times, due to the codepath in crown Core hitting
            ...
            if (blocks.size() > 0 && downloadData && blockChain != null) {
                ...
                for (InventoryItem item : blocks) {
                    if (blockChain.isOrphan(item.hash) && downloadBlockBodies) {
                        ...
                        //
                        // Note that as of June 2012 crown Core won't actually ever interleave blocks pushed as
                        // part of chain download with newly announced blocks, so it should always be taken care of by
                        // the duplicate check in blockChainDownloadLocked(). But crown Core may change in future so
    ...
    private void blockChainDownloadLocked(Sha256Hash toHash) {
        ...
        //
        //   https://en.crown.it/wiki/Protocol_specification#getblocks
        //
     ...
     * after the query returns. <b>Most peers do not support this request. You will need to connect to crown XT
     ...
     * <b>Most peers do not support this request. You will need to connect to crown XT peers if you want
     * this to work.</b>
     *
    ...
    public ListenableFuture<UTXOsMessage> getUTXOs(List<TransactionOutPoint> outPoints, boolean includeMempool) {
        ...
        try {
            ...
            if ((peerVer.localServices & GetUTXOsMessage.SERVICE_FLAGS_REQUIRED) != GetUTXOsMessage.SERVICE_FLAGS_REQUIRED)
                throw new ProtocolException("Peer does not support getutxos protocol flag: find crown XT nodes.");
