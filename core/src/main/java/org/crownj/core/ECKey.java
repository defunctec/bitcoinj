...
import org.crown.NativeSecp256k1;
import org.crown.NativeSecp256k1Util;
import org.crown.Secp256k1Context;
 ...
 *
 * <p>ECKey also provides access to crown Core compatible text message signing, as accessible via the UI or JSON-RPC.
 ...
 * are encoded using ASN.1/DER inside the crown protocol.</p>
 *
...
public class ECKey implements EncryptableItem {
    ...
    // The parameters of the secp256k1 curve that crown uses.
    ...
    /**
     * Construct an ECKey from an ASN.1 encoded private key. These are produced by OpenSSL and stored by crown
    ...
    /**
     * Output this ECKey as an ASN.1 encoded private key, as understood by OpenSSL or used by crown Core
     * in its wallet storage format.
     * @throws org.crownj.core.ECKey.MissingPrivateKeyException if the private key is missing or encrypted.
     */
    public byte[] toASN1() {
        try {
     ...
     * how ECDSA signatures are represented when embedded in other data structures in the crown protocol. The raw
    ...
    public static class ECDSASignature {
         ...
         * href="https://github.com/crown/bips/blob/master/bip-0062.mediawiki#Low_S_values_in_signatures">BIP62</a>.
         */
         ...
         * the same message. However, we dislike the ability to modify the bits of a crown transaction after it's
        ...
        public static ECDSASignature decodeFromDER(byte[] bytes) throws SignatureDecodeException {
            ...
            try {
                ...
                // crown signatures would not parse.
    ...
    /**
     * Signs the given hash and returns the R and S components as BigIntegers. In the crown protocol, they are
    ...
    /**
     * Signs a text message using the standard crown messaging signing format and returns the signature as a base64
     * encoded string.
     *
    ...
    /**
     * Given an arbitrary piece of text and a crown-format message signature encoded in base64, returns an ECKey
     * containing the public key that was used to sign it. This can then be compared to the expected public key to
     * determine if the signature was correct. These sorts of signatures are compatible with the crown-Qt/crownd
     * format generated by signmessage/verifymessage RPCs and GUI menu options. They are intended for humans to verify
     * their communications with each other, hence the base64 format and the fact that the input is text.
     *
     * @param message Some piece of human readable text.
     * @param signatureBase64 The crown-format message signature in base64
     ...
     * @throws org.crownj.core.ECKey.MissingPrivateKeyException if the private key bytes are missing/encrypted.
     */
    ...
    /**
     * Exports the private key in the form used by crown Core's "dumpprivkey" and "importprivkey" commands. Use
     ...
     * crowns controlled by the private key) you can use this method to check when you *encrypt* a wallet that
    ...
    /** The string that prefixes all text messages signed using crown keys. */
    private static final String crown_SIGNED_MESSAGE_HEADER = "crown Signed Message:\n";
    private static final byte[] crown_SIGNED_MESSAGE_HEADER_BYTES = crown_SIGNED_MESSAGE_HEADER.getBytes(StandardCharsets.UTF_8);

    /**
     * <p>Given a textual message, returns a byte buffer formatted as follows:</p>
     * <p>{@code [24] "crown Signed Message:\n" [message.length as a varint] message}</p>
     */
    private static byte[] formatMessageForSigning(String message) {
        try {
            ByteArrayOutputStream bos = new ByteArrayOutputStream();
            bos.write(crown_SIGNED_MESSAGE_HEADER_BYTES.length);
            bos.write(crown_SIGNED_MESSAGE_HEADER_BYTES);
