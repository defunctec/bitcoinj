...
public class TransactionOutput extends ChildMessage {
    ...
    // These fields are not crown serialized. They are used for tracking purposes in our wallet
    ...
    /**
     * <p>Gets the minimum value for a txout of this size to be considered non-dust by crown Core
     * (and thus relayed). See: CTxOut::IsDust() in crown Core.</p>
     *
     * <p>You probably should use {@link TransactionOutput#getMinNonDustValue()} which uses
     * a safe fee-per-kb by default.</p>
     *
     * @param feePerKb The fee required per kilobyte. Note that this is the same as crown Core's -minrelaytxfee * 3
     */
    public Coin getMinNonDustValue(Coin feePerKb) {
        ...
        long size = this.unsafecrownSerialize().length;
