...
import org.crownj.wallet.DefaultRiskAnalysis;
import org.crownj.wallet.KeyBag;
import org.crownj.wallet.RedeemData;
...
public class TransactionInput extends ChildMessage {
    ...
    protected void crownSerializeToStream(OutputStream stream) throws IOException {
        outpoint.crownSerialize(stream);
     ...
     * in nodes memory pools if the existing version is time locked. See the Contracts page on the crown wiki for
     ...
     * <a href="https://github.com/crown/bips/blob/master/bip-0068.mediawiki">relative lock-time</a> enabled.
     */
    ...
    public TransactionInput duplicateDetached() {
        return new TransactionInput(params, null, crownSerialize(), 0);
    }

    /**
     * <p>Returns either RuleViolation.NONE if the input is standard, or which rule makes it non-standard if so.
     * The "IsStandard" rules control whether the default crown Core client blocks relay of a tx / refuses to mine it,
