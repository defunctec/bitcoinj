...
package org.crownj.crypto;

import org.crownj.core.SignatureDecodeException;
...
public class TransactionSignature extends ECKey.ECDSASignature {
     ...
     * Because crown Core works via bit testing, we must not lose the exact value when round-tripping
     ...
     * crown Core. DER and the SIGHASH encoding allow for quite some flexibility in how the same structures
     ...
     * and generally violates people's mental model of how crown should work, thus, non-canonical signatures are now
     * not relayed by default.
     */
    public static boolean isEncodingCanonical(byte[] signature) {
        // See crown Core's IsCanonicalSignature, https://crowntalk.org/index.php?topic=8392.msg127623#msg127623
     ...
     * of the type used by crown we have to encode them using DER encoding, which is just a way to pack the two
     * components into a structure, and then we append a byte to the end for the sighash flags.
     */
    public byte[] encodeTocrown() {
        try {
     ...
     */
    public static TransactionSignature decodeFromcrown(byte[] bytes, boolean requireCanonicalEncoding,
            boolean requireCanonicalSValue) throws SignatureDecodeException, VerificationException {
        // crown encoding is DER signature + sighash byte.
        ...
        // In crown, any value of the final byte is valid, but not necessarily canonical. See javadocs for
