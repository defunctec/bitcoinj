...
import org.crownj.core.Base58;
import org.crownj.core.AbstractBlockChain;
import org.crownj.core.BlockChain;
import org.crownj.core.BloomFilter;
import org.crownj.core.Coin;
import org.crownj.core.Context;
import org.crownj.core.ECKey;
import org.crownj.core.FilteredBlock;
import org.crownj.core.InsufficientMoneyException;
import org.crownj.core.LegacyAddress;
import org.crownj.core.Message;
import org.crownj.core.NetworkParameters;
import org.crownj.core.Peer;
import org.crownj.core.PeerFilterProvider;
import org.crownj.core.PeerGroup;
import org.crownj.core.Sha256Hash;
import org.crownj.core.StoredBlock;
import org.crownj.core.Transaction;
import org.crownj.core.TransactionBag;
import org.crownj.core.TransactionBroadcast;
import org.crownj.core.TransactionBroadcaster;
import org.crownj.core.TransactionConfidence;
import org.crownj.core.TransactionInput;
import org.crownj.core.TransactionOutPoint;
import org.crownj.core.TransactionOutput;
import org.crownj.core.UTXO;
import org.crownj.core.UTXOProvider;
import org.crownj.core.UTXOProviderException;
import org.crownj.core.Utils;
import org.crownj.core.VerificationException;
import org.crownj.core.TransactionConfidence.*;
import org.crownj.crypto.*;
import org.crownj.script.*;
import org.crownj.script.Script.ScriptType;
import org.crownj.signers.*;
import org.crownj.utils.*;
import org.crownj.wallet.Protos.Wallet.*;
import org.crownj.wallet.WalletTransaction.*;
import org.crownj.wallet.listeners.CurrentKeyChangeEventListener;
import org.crownj.wallet.listeners.KeyChainEventListener;
import org.crownj.wallet.listeners.ScriptsChangeEventListener;
import org.crownj.wallet.listeners.WalletChangeEventListener;
...
import org.crownj.wallet.listeners.WalletReorganizeEventListener;
import org.slf4j.*;
 ...
 * of the crown protocol.</p>
 *
 * <p>To learn more about this class, read <b><a href="https://crownj.github.io/working-with-the-wallet">
 ...
 * other objects, see the <a href="https://crownj.github.io/getting-started">Getting started</a> tutorial
...
public class Wallet extends BaseTaggableObject
    ...
    // Dead:     Transactions that we believe will never confirm get moved here, out of pending. Note that crown
     ...
     * key, after loading a wallet produced by previous versions of crownj. If the wallet is encrypted the key
     ...
     * @throws org.crownj.core.ECKey.MissingPrivateKeyException if the seed is unavailable (watching wallet)
     */
    ...
    public void receivePending(Transaction tx, @Nullable List<Transaction> dependencies, boolean overrideIsRelevant) throws VerificationException {
        ...
        try {
            ...
            if (isTransactionRisky(tx, dependencies) && !acceptRiskyTransactions) {
                ...
                Transaction cloneTx = tx.getParams().getDefaultSerializer().makeTransaction(tx.crownSerialize());
    ...
    private void processTxFromBestChain(Transaction tx, boolean forceAddToPool) throws VerificationException {
        ...
        if (isDeadCoinbase) {
            ...
            // we could. crown Core nodes on the network have deleted the dependent transactions from their mempools
    ...
    /**
     * Returns the amount of crown ever received via output. <b>This is not the balance!</b> If an output spends from a
    ...
    /**
     * Returns the amount of crown ever sent via output. If an output is sent to our own wallet, because of change or
    ...
    public static class SendResult {
        /** The crown transaction message that moves the money. */
     ...
     *
     * @param address The crown address to send the money to.
     ...
     * @throws ExceededMaxTransactionSize if the resultant transaction is too big for crown to process.
    ...
    /**
     * Thrown if the resultant transaction is too big for crown to process. Try breaking up the amounts of value.
     */
    ...
    public void completeTx(SendRequest req) throws InsufficientMoneyException, BadWalletEncryptionKeyException {
        ...
        try {
            ...
            // Check size.
            final int size = req.tx.unsafecrownSerialize().length;
            ...
            for (Sha256Hash blockHash : oldBlockHashes) {
                for (TxOffsetPair pair : mapBlockTx.get(blockHash)) {
                    ...
                    if (tx.isCoinBase()) {
                        ...
                        // this coinbase tx. Some can just go pending forever, like the crown Core. However we
                        // can do our best.
        ...
        while (true) {
            ...
            for (int i = 0; i < req.tx.getOutputs().size(); i++) {
                TransactionOutput output = new TransactionOutput(params, tx,
                        req.tx.getOutputs().get(i).crownSerialize(), 0);
                if (req.recipientsPayFees) {
    ...
    private void addSuppliedInputs(Transaction tx, List<TransactionInput> originalInputs) {
        for (TransactionInput input : originalInputs)
            tx.addInput(new TransactionInput(params, tx, input.crownSerialize()));
    }
     ...
     * sending transactions to the crown network by default. Some sendCoins methods let you specify a broadcaster
     ...
     */
    public void setTransactionBroadcaster(@Nullable org.crownj.core.TransactionBroadcaster broadcaster) {
     ...
     * @throws org.crownj.wallet.DeterministicUpgradeRequiresPassword if key rotation requires the users password.
     */
    ...
    private Transaction rekeyOneBatch(long timeSecs, @Nullable KeyParameter aesKey, List<Transaction> others, boolean sign) {
        ...
        try {
            ...
            checkState(rekeyTx.unsafecrownSerialize().length < Transaction.MAX_STANDARD_TX_SIZE);
            return rekeyTx;
